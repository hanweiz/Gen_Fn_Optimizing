...
multp = np.loadtxt("Data/ALICE_Mult_7TeV_eta3.4_5.0.txt")
x_data = multp[:,0]
y_data = multp[:,1]
err = multp[:, 2]
...
p0 = [0.5399, 0.992, 2.298, 0.971, 2., 0.9] #for 1-component BD(NBD) X NBD
...
plt.plot(x_data, probconv(pbest, x_data), 'r-', label='$BD(NBD)$', linewidth=2)
plt.legend(loc='best')
...
#plt.figtext(0.13, 0.40, "$K2_{BD}=%.3f \pm %1.1e$ , $p2_{BD}=%.3f \pm %1.1e$" % (pbest[4], cov[4,4], pbest[5], cov[5,5]), fontsize=8)
#plt.figtext(0.13, 0.35, "$k2_{NBD}=%.3f\pm%1.1e$, $p2'=%.3f\pm%1.1e$" % (pbest[6], cov[6,6], pbest[7], cov[7,7]), fontsize=8)
...
#plt.savefig("ALICE7_3.4_5.0_1-compBD(NBD)_multiplicityfit.pdf", format='pdf')
